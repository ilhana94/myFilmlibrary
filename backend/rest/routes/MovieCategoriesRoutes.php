...
Flight::route('GET /movie-categories', function() {
    try {
        Flight::json((new \rest\services\MovieCategoriesServices())->getAll(), 200);  
    } catch (Exception $e) {
...
Flight::route('GET /movie-categories/@id', function($id) {
    try {
        $link = (new \rest\services\MovieCategoriesServices())->getById($id); 
        if ($link) {
...
Flight::route('POST /movie-categories', function() {
    try {
        ...
        Flight::json((new \rest\services\MovieCategoriesServices())->create($data), 201);  
    } catch (Exception $e) {
...
Flight::route('PUT /movie-categories/@id', function($id) {
    try {
        $data = Flight::request()->data->getData();
        $updated = (new \rest\services\MovieCategoriesServices())->update($id, $data); 
        if ($updated) {
...
Flight::route('DELETE /movie-categories/@id', function($id) {
    try {
        $deleted = (new \rest\services\MovieCategoriesServices())->delete($id); 
        if ($deleted) {
